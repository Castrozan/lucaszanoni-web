...
- **Scale-to-zero by construction.** Every in-repo app runs on Cloud Run with `min_instance_count = 0`, `cpu_idle = true`, fixed per-service CPU and memory limits, and a per-service `max_instance_count` (`infra/modules/serverless-cloud-run-app/main.tf`). An idle app costs nothing. See `infra/COST-CEILINGS.md`.
...
2. **Pre-create every referenced Secret Manager secret before the first apply.** If the entry's `origin.secretEnvironmentReferences` is non-empty, each referenced secret (the map value) and its `latest` version must already exist in the project, or the first `terraform apply` fails: the module both mounts the secret via `secret_key_ref { version = "latest" }` and grants `roles/secretmanager.secretAccessor` to the runtime service account, and both the Cloud Run revision and the IAM `setIamPolicy` return `NOT_FOUND` when the secret or version is absent. Create it out-of-band with `gcloud secrets create <secret-id> --project zg-url-shortener-2026` then `gcloud secrets versions add <secret-id> --data-file=<path-to-key>`. This is an owner action (it touches Secret Manager material outside the `production` Terraform state); the keyless CI cannot self-serve it. An entry with an empty `secretEnvironmentReferences` needs none of this.
3. **Pick the access model.** If the origin is `static-gcs-bucket`, the access model must be the public environment.
4. **Check whole-array uniqueness:** `id` and `mountPath` are globally unique; `cloudRunServiceName` and `appDirectoryName` are unique among in-repo entries; a `subdomain` serving location's `subdomainLabel` is unique. Enforced by `parseAppRegistry`'s `assertUnique` calls (`app-registry-parser.ts`).
5. **Do not touch the Worker or add a route, DNS record, or allowed-status list by hand.** The production projection locals derive the matching routing-table list, DNS record, and Worker route from the new entry and re-render `EDGE_ROUTES` on apply.
6. **Validate locally** through both gates: the Ajv schema test and `parseAppRegistry`. Run the config package vitest suite and the deploy `node --test` suites. Mirror the new live registry into the test fixtures (`packages/config/tests/app-registry-test-fixtures.ts`); `app-registry.spec.ts` asserts `appRegistry` deep-equals the fixtures list, so a registry change that skips the fixture update fails CI.
...
If the new entry's `origin.secretEnvironmentReferences` is non-empty, the referenced Secret Manager secret and its `latest` version must exist **before** `deploy-infrastructure` applies, not just before the app rollout. The first apply mounts the secret into the Cloud Run revision and grants the runtime accessor IAM in the same run; a missing secret or version fails that apply with `NOT_FOUND`. Pre-create it out-of-band as described in "Add an app" step 2 (`gcloud secrets create … && gcloud secrets versions add …`). The two `dynamic-ia-*` entries are the first to carry a secret reference (`GOOGLE_GENERATIVE_AI_API_KEY` → `dynamic-ia-gemini-api-key`), so their secret must be seeded once before their first apply.
