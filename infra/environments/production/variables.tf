...
variable "dynamic_ia_interfaces_container_image" {
  type        = string
  description = "Fully qualified Artifact Registry image for the dynamic-ia-interfaces service. Real app images are pushed out-of-band by CD; Terraform ignores image drift, so this seeds the first revision only."
  default     = "us-docker.pkg.dev/cloudrun/container/hello"
}

variable "stack_launcher_container_image" {
  type        = string
  description = "Fully qualified Artifact Registry image for the stack-launcher service. Real app images are pushed out-of-band by CD; Terraform ignores image drift, so this seeds the first revision only."
  default     = "us-docker.pkg.dev/cloudrun/container/hello"
}

variable "ingest_container_image" {
  type        = string
  description = "Fully qualified Artifact Registry image for the ingest service. Real app images are pushed out-of-band by CD; Terraform ignores image drift, so this seeds the first revision only."
  default     = "us-docker.pkg.dev/cloudrun/container/hello"
}
...
variable "enable_rin_session_tunnel" {
  type        = bool
  default     = false
  description = "Whether to provision the locally-managed Cloudflare Tunnel that publishes rin's own cockpit session bridge (serving its own tmux sessions directly) to the edge, so the workspace connects to rin without the chise SSH relay. Defaults off so a push applies no public origin; flipped on together with rin_session_tunnel_secret once the connector credentials are minted into rin's agenix store."
}

variable "rin_session_tunnel_secret" {
  type        = string
  sensitive   = true
  default     = ""
