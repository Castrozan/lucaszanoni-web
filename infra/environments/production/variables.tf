...
variable "enable_kira_session_tunnel" {
  type        = bool
  default     = false
  description = "Whether to provision the locally-managed Cloudflare Tunnel that publishes the owner-only cockpit session bridge on kira (the owner's laptop) to the edge, so the workspace lists and drives kira's real tmux claude sessions. Defaults off so a push applies no public origin; flipped on together with kira_session_tunnel_secret only once the connector credentials are minted into kira's agenix store."
}
